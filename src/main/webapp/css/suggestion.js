function sogouSugg(){
    ...
    var that = this,sugid,inputid,clearid,boxid,submitform,uri,sug_status,sugdata,input_elem,userInputString="";
    ...
    that.init = function(config){
        ...
        /** sugg **/
        var sugname = "sug";
        var suggTail = '<li class="last"><a class="s_close" href="javascript:window.'+sugname+'.close();">关闭</a></li>';
        var info={inputId:inputid,sugNum:5,sugScriptId:"sug",wrapper:boxid,form:submitform,resetbtn:clearid};
        ...
        function paint_result(W){
            ...
            for(var T=0;T<W.list.length&&T<info.sugNum;T++){
                ...
                if(Q != W.query){
                    tmp.push('<li ontouchstart="window.'+sugname+'.itemontouchstart(this)" onclick="window.'+sugname+'.select(\''+escape(Q)+'\','+(T+1)+',event);"><a class="s_b_1" href="javascript:" onclick="javascript:window.'+sugname+'.add(\''+escape(Q)+'\','+(T+1)+',event);"></a>'+V+'</li>')
                }
            }
            if(tmp.length>0){
                tmp.push(suggTail);
