package com.kate.app.model;

import java.sql.Timestamp;

public class AreaPeopleInfo2 {
	private int id;
	private String column1;//
	private String column2;//
	private String column3;//
	private int view_shunxu_people;//
	private String people_datasource;//
	private String people_date;//
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getColumn1() {
		return column1;
	}
	public void setColumn1(String column1) {
		this.column1 = column1;
	}
	public String getColumn2() {
		return column2;
	}
	public void setColumn2(String column2) {
		this.column2 = column2;
	}
	public String getColumn3() {
		return column3;
	}
	public void setColumn3(String column3) {
		this.column3 = column3;
	}
	public int getView_shunxu_people() {
		return view_shunxu_people;
	}
	public void setView_shunxu_people(int view_shunxu_people) {
		this.view_shunxu_people = view_shunxu_people;
	}
	public String getPeople_datasource() {
		return people_datasource;
	}
	public void setPeople_datasource(String people_datasource) {
		this.people_datasource = people_datasource;
	}
	public String getPeople_date() {
		return people_date;
	}
	public void setPeople_date(String people_date) {
		this.people_date = people_date;
	}
	
	
	
	

}
